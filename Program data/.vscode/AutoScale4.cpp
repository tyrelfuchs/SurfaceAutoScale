...
void setTabletScaling() {
    WinExec("C:\\AutoScale\\SetDpi.exe 1 200", SW_HIDE); // Set scaling to 200%
}

void setDesktopScaling() {
    WinExec("C:\\AutoScale\\SetDpi.exe 1 175", SW_HIDE); // Set scaling to 175%
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int) {
    ...
    while (true) {
        ...
        // Check tablet mode and monitor count if enabled
        if (isEnabled) {
            currentMode = isTabletMode();
            currentMonitorCount = getNumberOfMonitors();

            // Check for changes in tablet mode or monitor count
            if ((currentMode != lastMode) || (currentMonitorCount != lastMonitorCount)) {
                if (currentMode || currentMonitorCount == 1) {
                    setTabletScaling();
                } else {
                    setDesktopScaling();
                }
