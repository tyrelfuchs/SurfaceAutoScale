...
#pragma comment(lib, "user32.lib")   // Add this line
...
void setScaling(int percentage) {
    wchar_t command[256];
    // Use StringCchPrintfW for safer string formatting
    HRESULT hr = StringCchPrintfW(command, ARRAYSIZE(command), L"C:\\AutoScale\\SetDpi.exe 1 %d", percentage);
    if (SUCCEEDED(hr)) {
        HINSTANCE result = ShellExecuteW(NULL, L"open", command, NULL, NULL, SW_HIDE);
        if ((INT_PTR)result <= 32) {
            // Handle error (e.g., file not found, access denied)
        }
    } else {
        // Handle string formatting error
    }
}
    ...
    case WM_DISPLAYCHANGE: {
        ...
        if (isEnabled) {
            ...
            if (monitorCount > 1) {
                ...
                // Only built-in display is connected
                if (tabletMode) {
                    // In tablet mode
                    setScaling(200);
                } else {
                    // Not in tablet mode
                    setScaling(175); // Adjust this value if needed
                }
            }
        }
        break;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int) {
    ...
    bool lastTabletMode = currentTabletMode;
    ...
    while (true) {
        ...
        if (isEnabled) {
            currentMonitorCount = getMonitorCount();
            currentTabletMode = isTabletMode();

            if (currentMonitorCount != lastMonitorCount || currentTabletMode != lastTabletMode) {
                ...
                lastTabletMode = currentTabletMode;
            }
        }
